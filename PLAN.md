...
| 21 | Admin tooling | all | **IN PROGRESS** | Increments 1-3a of 4. **1:** V13, `AuditService` (SPEC 17.6 case 80, unclearable by construction), the `/ca` root, all eight SPEC 9.4.5 war commands — `verify` and `rollbackstatus` **make the SPEC 18.3 protocol runnable**. **2:** all of SPEC 9.4.1, the six SPEC 17.6 case 79 heuristics (**one could never have fired as SPEC words it**), RFC 4180 CSV export, click-to-inspect. **3a:** SPEC 9.4.3 claim admin — **`/ca claim protect` closes the last seam in the plugin**, open in `ClaimService.isAdminProtected` since M3, and its three consequences (unclaimable, unbuildable, war-immune per SPEC 11.6) are each tested separately. 1515 tests. **Remaining: 9.4.2 city, 9.4.4 economy, 9.4.6 system, SPEC 15.3 /report.** |
