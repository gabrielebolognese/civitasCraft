...
| 21 | Admin tooling | all | **IN PROGRESS** | Increment 1 of 4: V13 (`admin_protected_chunks`, `reports`), `AuditService` (SPEC 17.6 case 80, unclearable by construction rather than by a guard), the `/ca` root with its `/cityadmin` alias, and all eight SPEC 9.4.5 war commands — including `verify` and `rollbackstatus`, **which is what makes the SPEC 18.3 protocol runnable**. Admin paths on `WarService` are separate methods, never a bypass flag on the player path. 1471 tests. **Remaining: 9.4.1 inspection + fraud heuristics + CSV export, 9.4.2 city, 9.4.3 claim (closing the last M3 seam), 9.4.4 economy, 9.4.6 system, and SPEC 15.3 /report.** |
