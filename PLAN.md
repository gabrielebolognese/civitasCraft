...
**Next milestone:** #10 in the queue below, **M10, Outposts (rebuild)** — partly built; its row records exactly what is done and what is not.
...
| 10 | 10 | Outposts, rebuild | 41 | M3, M5, 3a | TODO | **Partly built, see below.** SPEC 39.1 to 39.9 in full. Multi-chunk claims, internal contiguity, merging, placement rules, distance-scaled upkeep and teleport, delinquency release order. Removes the superseded Part I M10.<br><br>**Done and committed:** `OutpostCostEngine` (29 tests against **every cell** of both SPEC 39.4 tables, SPEC 39.3's D(d) table and SPEC 39.5's upkeep/teleport table — SPEC 39.3's `n` is ambiguous and the tables settle it as the count *before* the purchase; the other reading overcharges 6%); `OutpostGeometry` (SPEC 39.6 contiguity incl. SPEC 18.1's ring cases, all three SPEC 39.7 merge outcomes, case 132); `cities.yml` carrying only the keys code reads. **SPEC 39.3 pricing is now what the service charges** (`creationCost` takes a position, because the same outpost costs 31,721 at a thousand blocks and 225,999 at a million), plus SPEC 39.6's 24-chunk outpost spacing and the multi-chunk accessors. Two Part I tests were replaced rather than deleted: both asserted rules SPEC 39.6 reverses. **Multi-chunk mechanics are built**: `expand` (F(k) pricing, adjacency, the four-chunk cap), `unclaimChunk` (refusing a split, deleting the outpost with its last chunk), `mergeAdjacentOutposts` (SPEC 39.7, older name wins), and `delete` refunding every chunk. **SPEC 39.7's outpost merge is unreachable on a default server** — 24-chunk spacing against a four-chunk cap — so it is tested with the spacing lowered rather than left unexercised.<br><br>**Established: no migration is needed.** `claims.outpost_id` is a foreign key, so several claims sharing one already *is* a multi-chunk outpost.<br><br>**Still to build:** multi-chunk `Outpost`/`OutpostRegistry`/`OutpostService`, SPEC 39.11's fifteen subcommands, the delinquency release order through `UpkeepCalculator`/`UpkeepTask`, folding `OutpostTeleport` into `TeleportService` (**three copies of warmup/cooldown consolidate here**), and removing 993 lines reached from twelve files. The rest of SPEC 39.15's config block lands with the service that reads it. |
