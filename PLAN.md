...
| 19 | War: lifecycle | M18, M13 | **IN PROGRESS** | Done: V12, the state machine (`WarState`, `War`, `WarZone`, `WarRegistry`), `WarService` (all 12 SPEC 11.3 preconditions + escrow + decline), `WarPayouts`, `WarScoring`, `WarPhaseTask` with outage catch-up, `Evacuation`, `WarRestrictions`, the no-drops listener, and **`RegistryWarZones` wired in — M17 now logs and M18 now restores for real**. Protection's grief and PvP seams closed. **Not done: capture points, applying the payouts, allies, item-theft log, living-entity restore, bounties, `/war`, Wars GUI, scoreboard, and 12 remaining seam closures.** |
