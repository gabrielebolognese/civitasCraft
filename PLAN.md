...
| 21 | Admin tooling | all | **IN PROGRESS** | Increments 1-3b of 4. **1:** V13, `AuditService` (SPEC 17.6 case 80, unclearable by construction), the `/ca` root, all eight SPEC 9.4.5 war commands — `verify` and `rollbackstatus` **make the SPEC 18.3 protocol runnable**. **2:** all of SPEC 9.4.1, the six SPEC 17.6 case 79 heuristics (**one could never have fired as SPEC words it**), RFC 4180 CSV export, click-to-inspect. **3a:** SPEC 9.4.3 — **`/ca claim protect` closes the last seam in the plugin**, open since M3; its three consequences tested separately. **3b:** all ten SPEC 9.4.2 city commands, V14 for the temporary upkeep multiplier. Admin paths are separate methods throughout, never flags — unfreezing a frozen city is the case that proves why. 1535 tests. **Remaining: 9.4.4 economy (incl. `/ca eco rollback`), 9.4.6 system, SPEC 15.3 /report.** |
