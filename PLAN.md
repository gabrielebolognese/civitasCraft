...
| 21 | Admin tooling | all | **IN PROGRESS** | Increments 1-2 of 4. **1:** V13, `AuditService` (SPEC 17.6 case 80, unclearable by construction), the `/ca` root and all eight SPEC 9.4.5 war commands — `verify` and `rollbackstatus` **make the SPEC 18.3 protocol runnable**. **2:** all of SPEC 9.4.1 (info, player, ledger x4, audit, inspect, alts, economy stats), the six SPEC 17.6 case 79 heuristics, RFC 4180 CSV export, click-to-inspect. **SPEC's 99th-percentile rule as written could never fire — the outlier was its own baseline; fixed and recorded.** 1502 tests. **Remaining: 9.4.2 city, 9.4.3 claim (closing the last M3 seam), 9.4.4 economy, 9.4.6 system, SPEC 15.3 /report.** |
