...
| 21 | Admin tooling | all | **IN PROGRESS** | Increments 1-4a. **1:** V13, `AuditService`, the `/ca` root, SPEC 9.4.5's war commands — `verify` and `rollbackstatus` **make the SPEC 18.3 protocol runnable**. **2:** SPEC 9.4.1, the six SPEC 17.6 case 79 heuristics (**one could never have fired as SPEC words it**), CSV export, inspect. **3a:** SPEC 9.4.3 — **`/ca claim protect` closed the last seam in the plugin**. **3b:** all ten SPEC 9.4.2 city commands, V14 upkeep multiplier. **4a:** SPEC 9.4.4 economy — `/ca eco rollback` writes compensating entries and never deletes (SPEC 3.6), floors at zero and records debt (SPEC 17.3 case 35), and reports downstream rather than cascading onto innocent third parties. 1548 tests. **Remaining: 9.4.6 system and SPEC 15.3 /report.** |
