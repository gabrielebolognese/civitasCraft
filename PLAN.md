...
| 19 | War: lifecycle | M18, M13 | **IN PROGRESS** | Done: V12, `WarState`, `WarZone`, `War`, `WarRegistry`, `WarService` (all 12 SPEC 11.3 preconditions + escrow + decline), `WarPayouts`, `WarScoring`, both SPEC 18.1 tests. **Not done: phase task, evacuation, capture points, resolution payout application, allies, combat listeners, no-drops, bounties, `/war`, Wars GUI, scoreboard, and all 14 seam closures — `WarZones` is still `none()`, so M17 logs nothing and no war can actually be fought yet.** |
